...
@interactions.slash_command(name="ping", description="Ping command", scope=guild_ids)
async def ping(ctx: interactions.SlashContext):
    await ctx.send("Pong!")

@interactions.slash_command(name='lowestbin', description='Lowest bin command', scope=guild_ids, options=[
